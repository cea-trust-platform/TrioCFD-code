...
if (1):
    ...
    part.setScalarColormap("SCALAR", "ELEM", -1,1, colormap);
    part.setPointSize(0.0005)
    part.setLightFactor(1)
    part.set_cache_all_fields(0)
    part.set_repeat_database(1)
    if (partmode==1):
        part.setMaxlifetime(10000,10000,1e-10)
        part.set_particles_per_second(npartmax*0.01)
    else:
        part.setMaxlifetime(30,1,0.00001)
        part.set_particles_per_second(1e9)
        pass
    part.set_integration_step(0.1)
    part.set_motion_blur_duration(0.0001)
    
    srcs=[]
    srcs.append(ParticleSourceBox())
    if (partmode==1):
        #srcs[0].setBox(0, 0, 0.75, 0.05, 1, 0.75)
        srcs[0].setBox(0.0005, 0.05, 0.0, 0.0005, 0.05, 0.12)
	srcs[0].setGrid(0,0,0)
    else:
        srcs[0].setBox(0, 0.003, 0., 0.2, 0.003, 0.12)
        pass
    srcs[0].setWeight(1)
...
engine.set_speed_ratio(1)
engine.set_current_time(0)
