...
//
// File:        Parser.cpp
// Directory:   $TRIO_U_ROOT/Kernel/Math
// Version:     /main/33
//
//////////////////////////////////////////////////////////////////////////////
...
/* Les identificateurs suivants doivent etre d�finis de mani�re unique pour chaque fonction */
...
double Parser::evalFunc(PNode* node)
{
  ...
  if (node->value<=0)
    {
      UnaryFunction& f = unary_func[-node->value-1]; // OC attention, dans node->value c est l'oppos� de l'indice de la func dans la liste 
      // afin de distinguer op�rateur binaire (>0) et fonctions unaires (<0>
      // Il est donc necessaire de prendre -node->value ici pour r�f�rencer un �l�ment de la liste
...
// Ne pas inliner car sinon Parser::eval(PNode* node) plus souvent appel�e encore
...
double Parser::evalOp(PNode* node)
{
  ...
  switch (node->value)
    ...
    case 2: // MULTIPLY
      return x * y;
    case 3: // DIVIDE
      if (y==0) {Cerr << "Error in the Parser : x/y calculated with y equals 0. You are using a formulae with a division per 0." << finl;Process::exit();}
      return x/y;
    case 4: // POWER
      if (y != int(y) && x<0) {Cerr << "Error in the Parser : x^y calculated with negative value for x and y real." << finl;Process::exit();}
      return pow(x,y);
    case 5: // LT
      return (x<y)?1:0;
    case 6: // GT
  ...
  if (tokenizer->type == StringTokenizer::STRING)
    {
      ...
      if (trouv>-1)
      ...
      else
        {
          ...
          if (trouv>-1)
            {
              op->push(-trouv); // OC 01/2005 : Attention, on stocke l'oppos� de l'indice trouv� afin de bien dissocier les op�rateurs binaires des fonctions unaires.
              state = 0;
            }
          else 
            {
              ...
              if (trouv>=0)
              ...
              else 
                {
                  ...
                  Cerr << *str << "\n";
                  Cerr << " identifier " << func << " unknown , not in" << finl;
		   for (int i=0;i<ivar;i++)
  ...
  for (auto& itr : unary_func)
    {
      ...
      Nom n2(f.getName());
      n2.majuscule();
      if (nv == n2) return i+1 ; // OC: pour ne pas utiliser le zero car sinon conflit avec la nouvelle num�rotation des op�rateurs binaires.
      i++;
    }
  //Non ca marche pas encore le pere, car la numerotation des fcts n est pas la meme entre pere et fils !!
  /*  if (pere != NULL)
      {
      Cout << " father nonzero" << finl;
      return pere->searchFunc(v);
      }*/
  return -1;
}

void Parser::addCst(Constante& cst)
{
  les_cst.add(cst);
}
        
void Parser::setPere(Parser& p)
{
  pere = &p;
}
        
void Parser::addFunc(UnaryFunction& f)
{
  for (int i=0;i<unary_func.size();i++)
    {
