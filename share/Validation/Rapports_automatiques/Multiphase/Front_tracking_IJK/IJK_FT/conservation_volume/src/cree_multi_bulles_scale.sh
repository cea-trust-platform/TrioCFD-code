...
cat $liste_bulles |
{
  read x y z mesh scale
  while test -n "$mesh"
  do
    echo bulle $x , $y , $z , $mesh , $scale
    export x
    export y
    export z
    export mesh
    ...
    echo $1 $mesh $nelem
    awk 'BEGIN{x=ENVIRON["x"];y=ENVIRON["y"];z=ENVIRON["z"];scale=ENVIRON["scale"]}
         $1~"EndNodes"{flag=0}
	 flag==2{print x+($2*scale),y+($3*scale),z+($4*scale)}
	 flag==1{n=$1;flag=2}
	 $1~"Nodes"&&(!($1~"EndNodes")){flag=1}' $mesh>>$nodefile
