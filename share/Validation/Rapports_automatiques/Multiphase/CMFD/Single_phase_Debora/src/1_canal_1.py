...
#        r_ = (coo[i, 1] * n/(n+1) + 1/(n+1) ) * r
    ...
    z_b, z_t = 0, h
    g_t, g_b, g_wc, g_wh, g_s = [], [], [], [], [] #top, bottom, wall, symmetry
    for i, b in enumerate(bf):
        ...
        elif (F_Ei[i + 1] == F_Ei[i] + 1) and (b[2] > 1.) and (b[2] < 4.5)  : g_wh.append(i)
        elif (F_Ei[i + 1] == F_Ei[i] + 1) : g_wc.append(i)
    ...
    # ajout des groupes
    for (n, g) in [("top", g_t), ("bottom", g_b),  ("wall_hot", g_wh),  ("wall_cold", g_wc), ("symetrie", g_s)]:
