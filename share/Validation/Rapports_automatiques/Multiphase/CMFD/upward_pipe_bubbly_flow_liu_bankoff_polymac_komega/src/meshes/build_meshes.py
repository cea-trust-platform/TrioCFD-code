...
def build_mesh_liu_1(alpha_d, folder, med_name):
    # rayon maillage
    r = 0.019
    # hauteur maillage
    h = 1.52
    # nombre mailles radiales et axiales
    n = 20
    nz = 80
    taux = 0.9  # taux de croissance de la taille des mailles

    theta_mesh_size = 45  # en degres
    nt = ceil(
        alpha_d / theta_mesh_size
    )  # equivalent axisymetrique (une maille 3D d'epaisseur)
    ...
    y = [(1 - taux**i) / (1 - taux**n) for i in range(n + 1)]
    ...
    for i, b in enumerate(bf):
        if abs(b[2] - z_t) < 1e-5:
            g_t.append(i)
        elif abs(b[2] - z_b) < 1e-5:
            g_b.append(i)
        elif b[1] < 1.0e-8:
            g_s.append(i)
        elif (b[0] > 1.0e-8) and (abs(alpha - np.arctan(b[1] / b[0]))) < 1e-5:
            g_s.append(i)  # use of pi/2-angle because we know b[1]>0
        elif F_Ei[i + 1] == F_Ei[i] + 1:
            g_w.append(i)
...
def build_mesh_liu_2(alpha_d, folder, med_name):
    # rayon maillage
    r = 0.019
    # hauteur maillage
    h = 1.52
    # nombre mailles radiales et axiales
    n = 25  # noeuds direction radiale
    nz = 80
    taux = 0.9  # taux de décroissance de la taille des mailles depuis l'origine
...
if __name__ == "__main__":
    build_mesh_liu_1(5.0, ".", "mesh_Liu_1.med")
    build_mesh_liu_2(5.0, ".", "mesh_Liu_2.med")
