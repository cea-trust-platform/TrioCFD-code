...
seq $nelem | awk 'BEGIN{x=ENVIRON["connex_compo"]}{print x}'>>$compofile
  ...
  echo TEMPS 0 >>$lata
  echo Geom INTERFACES type_elem=TRIANGLE_3D >>$lata
  echo Champ SOMMETS $nodefile geometrie=INTERFACES size=$nnodestot composantes=3 >>$lata
  echo Champ ELEMENTS $elemfile geometrie=INTERFACES size=$nelemtot composantes=3 FORMAT=INT32 >>$lata
  echo Champ COMPO_CONNEXE $compofile geometrie=INTERFACES size=$nelemtot composantes=1 FORMAT=INT32,NO_INDEXING localisation=ELEM >>$lata
  echo FIN >>$lata
}
exit
...
echo TEMPS 0 >>$lata
echo Geom INTERFACES type_elem=TRIANGLE_3D >>$lata
echo Champ SOMMETS $nodefile geometrie=INTERFACES size=$nnodes composantes=3 >>$lata
echo Champ ELEMENTS $elemfile geometrie=INTERFACES size=$nelem composantes=3 FORMAT=INT32 >>$lata
echo Champ COMPO_CONNEXE $compofile geometrie=INTERFACES size=$nelem composantes=1 FORMAT=INT32,NO_INDEXING >>$lata
echo FIN >>$lata
