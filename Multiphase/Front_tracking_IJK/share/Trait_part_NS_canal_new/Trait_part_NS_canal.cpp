...
Entree& Traitement_particulier_NS_canal::lire(Entree& is)
{
  ...
  if(sub_type(Navier_Stokes_Turbulent,mon_equation.valeur()))  { oui_profil_nu_t = 1 ; Nval=13 ; }
...
void Traitement_particulier_NS_canal::remplir_Tab_recap(DoubleTab& Tab_recap)
{
// surchargeée dans VDF
}
...
void Traitement_particulier_NS_canal::remplir_reordonne_Y_tot(const DoubleVect& Y, DoubleVect& Y_tot) const
{
    DoubleVect Y_p(Y);
    entier j_tot=1;
	  ...
	  for (int k=0; k<Y_p.size(); k++)
	    {
	      if(!est_egal(Y_p(k),-100.)) // on recherche si Y_p(k) est deja contenu dans Y_tot
	      {
	        ...
	        for (int j=0;j<Y_tot.size(); j++) { if (est_egal(Y_tot(j),Y_p(k))) ok_new=0; }
		  
		if(ok_new==1)
		  {
		   Y_tot.resize(j_tot+1);
		   Y_tot(j_tot) = Y_p(k);
		   j_tot++;
		  }  
	      }
...
void Traitement_particulier_NS_canal::reprendre_stat_canal(DoubleTab& val, const Nom& fichier)
{
   ...
   if(fic.fail()) { Cerr << "Impossible d'ouvrir le fichier " << fichier << finl; exit(); }
...
void Traitement_particulier_NS_canal::post_traitement_particulier()
{
  ...
  if(je_suis_maitre())
  {
	  ...
	  {
	    static entier init_stat_temps = 0;
  ...
  ///////////////////////////////////////////////////////////////
    ...
    if(je_suis_maitre())
    {
       ...
       else
       {
          ...
          entier i=(entier) (tps/dt_impr_moy_spat + epsilon);
          entier j=(entier) (tps_passe/dt_impr_moy_spat + epsilon);
          impr_inst=(i>j);
       }
       entier impr_stat;
       ...
       {
          double epsilon = 1.e-8;
          entier i=(entier) (tps/dt_impr_moy_temp + epsilon);
          entier j=(entier) (tps_passe/dt_impr_moy_temp + epsilon);
          impr_stat=(i>j);
       }
...
void Traitement_particulier_NS_canal::calcul_reynolds_tau()
{
 ...
 // !!!!!  Hypotheses : maillage symetrique suivant la demi-hauteur et s'étendant de Y=0 a Y=H
  ...
  entier kmax=NN-1-k;		    // indice du dernier point hors paroi
  ...
  //////////////////////////////////////////////////////
   ...
   if (sub_type(Navier_Stokes_Turbulent,mon_equation.valeur()))
	...
	if(Process::nproc()>1) // PQ : 11/06/07
	{
	 ...
	 Cerr << " n'est pas parallélisé - A FAIRE " << finl;
	 Cerr << " autre possibilité : mettre les parois sur le même proc" << finl;
	 Cerr << " et commenter la présente verif dans Traitement_particulier_NS_canal::calcul_reynolds_tau   " << finl;
	 exit();
	}
	  ...
	  entier nbfaces = la_front_dis.nb_faces();
          entier ndeb = la_front_dis.num_premiere_face();
          entier nfin = ndeb + nbfaces;

        if (sub_type(Dirichlet_paroi_fixe,la_cl.valeur())) 
         {
	    if (Objet_U::dimension == 2 )
	      {
            	for (fac=ndeb; fac<nfin ; fac++) { tauwb+=sqrt(tau_tan(fac,0)*tau_tan(fac,0)); } 
	      }
	     else
	      {
            	for (fac=ndeb; fac<nfin ; fac++) { tauwb+=sqrt(tau_tan(fac,0)*tau_tan(fac,0)+tau_tan(fac,2)*tau_tan(fac,2)); } 
	      }
	  ...
	  entier nbfaces = la_front_dis.nb_faces();
	    ...
	    if (Objet_U::dimension == 2 )
	      {
            	for (fac=ndeb; fac<nfin ; fac++) { tauwh+=sqrt(tau_tan(fac,0)*tau_tan(fac,0)); } 
	      }
	     else
	      {
            	for (fac=ndeb; fac<nfin ; fac++) { tauwh+=sqrt(tau_tan(fac,0)*tau_tan(fac,0)+tau_tan(fac,2)*tau_tan(fac,2)); } 
	      }
  ...
  //////////////////////////////////////////////////////
      ...
      utaub=sqrt(tauwb/rho_bas);  if(val_moy_tot(kmin,0,0)<=0) utaub*=-1.;
      utauh=sqrt(tauwh/rho_haut); if(val_moy_tot(kmax,0,0)<=0) utauh*=-1.;
...
void Traitement_particulier_NS_canal::ecriture_fichiers_moy_Temp(const DoubleTab& val_moy, const Nom& fichier, const double& dt, const int& k) const
{
      ...
      double tps = mon_equation->inconnue().temps();
      double u,v,w,T,T2,uT,vT,wT,kappa,rhoT,rhouT,rhovT,rhowT,rhou,rhov,rhow,rhouu,rhovv,rhoww,rhouv,rhouw,rhovw,gradT,fluxdiff,fluxsm; //modified YB 30/06/09
      int i;
...
void Traitement_particulier_NS_canal::ecriture_fichiers_moy_vitesse_rho_mu_old(const DoubleTab& val_moy, const Nom& fichier, const double& dt, const int& k) const
{
      ...
      double tps = mon_equation->inconnue().temps();
      double u,v,w,u2,v2,w2,uv,uw,vw,rho,mu; //modified AT 5/06/09
      int i;
