...
"""
    Evoluton de l'energie et de la dissipation pour differents cas sur le meme plot.
"""

########################################################################
# mode d'emploi : python energy_fusion_cas.py nom_fig liste_chemin_txt liste_signes   directions  N_smooth [temps0,temps1] correction_listes
#                            0                   1           2               3             4         5            6
...
message = "#############################################\n\
l'energie et la dissipaiton au cours du temps \n\
##############################################"
print(message)
...
liste_chemin_txt = sys.argv[2].rstrip("]").lstrip("[").split(",")
good_liste_txt = []
BIG_dix = int(sys.argv[7])
print("BIG_dix // correction : ", BIG_dix)
if BIG_dix:
  for out in liste_chemin_txt:
    if (("N01" in out) or ("SIMU_120" in out)) :
        good_liste_txt.append(out)
  liste_chemin_txt = good_liste_txt
...
print(" len(liste_chemin_txt) : ",len(liste_chemin_txt))
if len(liste_chemin_txt)>1:
...
for ind, chemin_txt in enumerate(liste_chemin_txt):
    print("fichier out : "+chemin_txt)
    t_deb, t_fin = float(sys.argv[6].rstrip("]").lstrip("[").split(",")[-2]),float(sys.argv[6].rstrip("]").lstrip("[").split(",")[-1])
    print("# t_deb = %s \n# t_fin = %s"%(str(t_deb),str(t_fin)))
    
    print(numpy.loadtxt(glob.glob(chemin_txt)[-1]).shape)
    temps = numpy.loadtxt(glob.glob(chemin_txt)[-1])[:,1]
    ########################################################################
    
    
    ## Lecture, selection, rangement et traitement des donnees
    # chemin_txt = "/volatile/RANDOM_TRIPERIO/a030/RE_ISOLE_143/RUN05/OUT/"
    
    bilan_qdm = numpy.loadtxt(glob.glob(chemin_txt)[-1])#[rosette["r_vel"]["x"]][it_deb:it_fin,1:]
