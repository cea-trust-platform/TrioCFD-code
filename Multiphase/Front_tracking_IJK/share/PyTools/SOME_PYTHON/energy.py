...
################################################################################################
def write_me_in_raw(array_to_write,array_name):
            the_shapes_of_array_to_write = array_to_write.shape
            the_shapes_of_array_to_write.tofile("the_shapes_of"+array_name+".raw")
            array_to_write.tofile(array_name+".raw")
            del(array_to_write)
################################################################################################
...
if phase!="mel":
    for f in fstat:
        ...
        if ((t[-1]>=t_deb) and (t[-1]<=t_fin)): 
            ...
            print("m_u_ll : ",m_u_ll.shape)
            print("u_l_u_l : ",u_l_u_l.shape)
            print("m_Rij_ll : ",m_Rij_ll.shape)
            # 
            ...
            if phase=='liq': 
                    ...
                    e_m = -1*nu*(np.einsum("ij...,ij...->...",m_sij_ll,m_sij_ll))  # double-produit contracte de sij moyen.
                    true_e_m = e_m
                    e_f = e_t-e_m
                    true_e_f = true_e_t-e_m
...
mKF = np.mean(np.array(KF[-int(len(KF)/2):])) # moyenne de l'Ec de fluctuations : <u'.u'>
...
if draw_the_graphs:
    ...
    if adim:
        ...
        ax.legend(fontsize=22)
        plt.tight_layout()
        fig.savefig(figname+"_K_D.png")
        fig.savefig(figname+"_K_D.pdf")
    ...
    if phase=='liq':   
        ...
        if N_smooth > 0:
            # la longeur du plus petit vecteur dicte combien de points de trace on a 
            n_max = min(len(t),len(np.convolve( T_ET,filtre,mode='same')))    
            # Adoucissons le signal
            ...
            ax.plot(t_t[:],T_EM[:],'--',label='moyen : ($\nu \overline{s_{ij}} \cdot \overline{s_{ij}}$)')
            ax.plot(t_t[:],T_EF[:],'-.',label='fluct. : ($\nu \overline{s\' _{ij} \cdot s\' _{ij} }$) $\sim$' +str(np.round(mT_EF*10**(-T_E_coeff),5))+r'$\cdot 10^{%s}$'%(T_E_coeff))
            if diphasique:
        ...
        ax.legend(fontsize=22)
        plt.tight_layout()
        fig.savefig(figname+"_TrDissip.png")
        fig.savefig(figname+"_TrDissip.pdf")
    ...
    if adim:
        ...
        ax.legend(fontsize=22)
        plt.tight_layout()
        fig.savefig(figname+"_K_TrD.png")
        fig.savefig(figname+"_K_TrD.pdf")
    ...
    write_me_in_raw(temps_KT_KTB_KM_KF,"temps_KT_KTB_KM_KF")
    ...
    if phase=='liq':
        ...
        if N_smooth > 0:
            ...
            write_me_in_raw(temps_ET_EM_EF,"temps_ET_EM_EF")
    ...
    if adim:
        temps_EF_KF = np.array(t[:],
                               EF[:], 
                               KF[:])
        write_me_in_raw(temps_EF_KF,"temps_EF_KF")
    ...
    if phase=='liq':   
        ...
        if N_smooth > 0:
            ...
            # N'adoucissons pas le signal
            temps_T_ET_T_EM_T_EF = np.array(t_t[:],
                                            T_ET[:],
                                            T_EM[:],
                                            T_EF[:])
            write_me_in_raw(temps_T_ET_T_EM_T_EF,"temps_T_ET_T_EM_T_EF")    
    ...
    if adim:
        temps_T_EF_KF = np.array(t[:],
                                 T_EF[:],
                                 KF[:])
        write_me_in_raw(temps_T_EF_KF,"temps_T_EF_KF")
