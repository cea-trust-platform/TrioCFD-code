...
if draw_the_graphs:
    ...
    def get_K_label(phase):
        ...
        label_KF  = r"fluct : $ \frac{1}{2} (u' \cdot u'$); " + r'$\langle{K_F} \rangle = $ '+str(np.round(mF*10**(-mF_coeff),5))+r'$\cdot 10^{%s}$'%(mF_coeff)
    ...
    def do_the_K_plot(phase):
        ...
        if N_smooth > 0:
            ...
            ax.plot(t[:n_max],np.convolve(TOT_bis,filtre,mode='same')[:n_max],'+r',label=label_TB)
            ...
            ax.plot(t[:],TOT_bis[:],'+r',label=label_TB)
    ...
    def get_E_label(phase):
        ...
        label_EF = (r'fluct. : $ \nu \overline{\partial_i u^p _j \partial_j u^p _i} $) $\sim$'+
                    str(np.round(mF*10**(-coeff),5))+r'$\cdot 10^{%s}$'%(coeff))
        if diphasique : label_Pr = (r'$\mathcal{P} : \alpha \Delta \rho g u_r \sim $'
                    +str(np.round(mPr*10**(-coeff),5))+r'$\cdot 10^{%s}$'%(coeff))
        else: label_Pr=""
    ...
    def do_EK_plot(phase,which_one):
        if adim:
            ...
            ax.plot(t[:],EF[:],'-',color='red',label=label_EF)
            ax.plot(t[:], KF[:],'-',color='black',label=label_KF)
    ...
    # ~~~ OBSOLETE - VRAIE DISSIPATION TURBULENTE \epsilon ~~~~~~~~~~~~~~~~~~~~~~~~
   # if phase=='liq':   
   #     fig, ax = plt.subplots(1,figsize=(9,7))
   ...
   #         # Adoucissons le signal
   #         ax.plot(t_t[:n_max],np.convolve(T_ET, filtre, mode='same')[:n_max],'k', label=label_T_ET)
   #         ax.plot(t_t[:n_max],np.convolve(T_EM, filtre, mode='same')[:n_max],'--',label=label_T_EM)
   #         ax.plot(t_t[:n_max],np.convolve(T_EF, filtre, mode='same')[:n_max],'-.',label=label_T_EF)
   #         if diphasique:
   ...
   #         ax.plot(t_t[:],T_EF[:],'-.',label=r"fluct. : ($\nu \overline{s^p _{ij} \cdot s^p _{ij} }$) $\sim$" +str(np.round(mT_EF*10**(-T_E_coeff),5))+r'$\cdot 10^{%s}$'%(T_E_coeff))
   #         if diphasique:
   ...
   #     fig.savefig(chemin_sauvegarde+figname+"_TrDissip.pdf")
    ...
    # ~~ OBSOLETE - EC ET VRAIE DISSIPATION TURBULENTE SUR LE MM GRAPHIQUE ~~~~~~~~
   # if adim:
   #     fig, ax = plt.subplots(1,figsize=(9,7))
   #     ax.set_title(r'Moyenne '+phase,fontsize=18)
   # 
   #     ax.plot(t[:],T_EF[:],'-',color='red', label=label_T_EF)
   #     ax.plot(t[:], KF[:],'-',color='black',label=label_KF)
   # 
   #     ax.set_xlabel(r't / $ T_L ; T_L = \frac{\langle{K_F} \rangle }{\langle{\epsilon _f} \rangle} = $'+str(np.round(T_L,2)),fontsize=18)
   #     ax.set_ylabel(
...
def do_the_write_the_raws(phase):
    if "mel" in phase:
        ...
        my_KF_coeff, my_coeff, my_mKF, my_mKF_coeff = KF_coeff, K_coeff, mKF, mKF_coeff
    ...
    if "liq" in phase:
        ...
        my_KF_coeff, my_coeff, my_mKF, my_mKF_coeff = KF_coeff_l, K_coeff_l, mKF_l, mKF_coeff_l
    ...
    if "vap" in phase:
        ...
        my_KF_coeff, my_coeff, my_mKF, my_mKF_coeff = KF_coeff_v, K_coeff_v, mKF_v, mKF_coeff_v
    ...
    KF_coeff__coeff__mKF__mKF_coeff = np.array((my_KF_coeff, my_coeff, my_mKF, my_mKF_coeff)) 
    ...
    if N_smooth > 0:
        # la longeur du plus petit vecteur dicte combien de points de trace on a 
        n_max = min(len(t),len(np.convolve( my_KT,filtre,mode='same'))-N_smooth)
        # On sait que les energies et dissipation sont très bruitees : on va les adoucir
        temps_KT_KTB_KM_KF = np.array((t[:n_max],
                                      np.convolve( my_KT, filtre,mode='same')[:n_max],
                                      np.convolve( my_KTB,filtre,mode='same')[:n_max],
                                      np.convolve( my_KM, filtre,mode='same')[:n_max],
                                      np.convolve( my_KF, filtre,mode='same')[:n_max]))
    ...
    write_me_in_raw(chemin_sauvegarde,temps_KT_KTB_KM_KF,"temps_KT_KTB_KM_KF_"+phase)
    
    # ENERGIE CINETIQUE DES FLUCTUATIONS (si KM et KF sont tres differents uniquement)
    if (np.mean(np.array(my_KM[-int(len(my_KM)/2):]))/np.mean(np.array(my_KF[-int(len(my_KF)/2):])) > 6.):
    ...
    if phase=='liq':
        ...
        if N_smooth > 0:
            # la longeur du plus petit vecteur dicte combien de points de trace on a 
            n_max = min(len(t),len(np.convolve( my_ET,filtre,mode='same'))-N_smooth)    
            # Adoucissons le signal
            temps_ET_EM_EF = np.array((t[:n_max],
                                      np.convolve(my_ET, filtre, mode='same')[:n_max],
                                      np.convolve(my_EM, filtre, mode='same')[:n_max],
                                      np.convolve(my_EF, filtre, mode='same')[:n_max]))
            write_me_in_raw(chemin_sauvegarde,temps_ET_EM_EF,"temps_ET_EM_EF_"+phase)


            if diphasique:
                temps_Pr = np.array((t[:n_max],np.convolve(Pr, filtre, mode='same')[:n_max]))
                write_me_in_raw(chemin_sauvegarde,temps_Pr,"temps_Pr_"+phase)
            # ax.text(0.99*(t[-1]+t[0])/2, 0.3*mEF, r'$\mathcal{P} \sim \alpha \Delta \rho g u_r =$ '+str(np.round(mPr*10**(-E_coeff),5))+r'$\cdot 10^{%s}$'%(E_coeff), fontsize=18)
        else:
            # N'adoucissons pas le signal
    ...
    if adim:
        temps_EF_KF = np.array((t[:],my_EF[:],my_KF[:]))
        write_me_in_raw(chemin_sauvegarde,temps_EF_KF,"temps_EF_KF"+phase)
    ...
    if N_smooth > 0:
        # la longeur du plus petit vecteur dicte combien de points de trace on a 
        n_max = min(len(t),len(np.convolve( my_T_ET,filtre,mode='same'))-N_smooth)    
        # Adoucissons le signal
        temps_T_ET_T_EM_T_EF = np.array((t_t[:n_max],
                                        np.convolve(my_T_ET, filtre, mode='same')[:n_max],
                                        np.convolve(my_T_EM, filtre, mode='same')[:n_max],
                                        np.convolve(my_T_EF, filtre, mode='same')[:n_max]))
                                        
        if diphasique:
            temps_Pr = np.array((t_t[:n_max],np.convolve(T_Pr, filtre, mode='same')[:n_max]))
            write_me_in_raw(chemin_sauvegarde,temps_Pr,"temps_Pr_"+phase)
    else:
        # N'adoucissons pas le signal
        temps_T_ET_T_EM_T_EF = np.array((t_t[:],my_T_ET[:],my_T_EM[:],my_T_EF[:]))
    write_me_in_raw(chemin_sauvegarde,temps_T_ET_T_EM_T_EF,"temps_T_ET_T_EM_T_EF_"+phase)    
    
    if diphasique:
        temps_Pr = np.array((t_t[:],T_Pr[:]))
        write_me_in_raw(chemin_sauvegarde,temps_Pr,"temps_Pr_"+phase)
        
        
    if adim:
        temps_T_EF_KF = np.array((t[:],my_T_EF[:],my_KF[:]))
        write_me_in_raw(chemin_sauvegarde,temps_T_EF_KF,"temps_T_EF_KF_"+phase)

if "mel" in phase:
    do_the_write_the_raws(phase)
if "liq" in phase:
...
def do_the_final_print(phase):
    ...
    if diphasique:
        print(r'$\mathcal{P} : \alpha \Delta \rho g u_r \sim $'+str(np.round(mPr*10**(-my_E_coeff),5))+r'$\cdot 10^{%s}$'%(my_E_coeff))
        print(" ==> P_inj = ",(np.round(mPr*10**(-my_E_coeff),5))*10**(my_E_coeff))
    # print("Estimation de la pseudo-dissipation turbulente : ")
    # print(r'$ \epsilon = $',(np.round(my_E_ylabs*10**(-my_E_coeff),3)))
    # print("Estimation de la vraie dissipation turbulente : ")
    # print(r'$ \epsilon = $',(np.round(my_T_E_ylabs*10**(-my_T_E_coeff),3)))
    print("Moyenne de la pseudo-dissipation, calculée sur < t < ")
    ...
    print("Moyenne de la vraie dissipation, calculée sur < t < ")
