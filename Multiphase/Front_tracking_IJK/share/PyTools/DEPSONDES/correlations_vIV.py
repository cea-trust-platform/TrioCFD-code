...
"""
## Lecture des donnees utilisateur _____________________________________
print("#######")
print(sys.argv[1])
print(sys.argv[2])
print(sys.argv[3])
...
## Creation des chemins, recuperation du temps _________________________
...
## Recuperation des iterations a considerer dans les fichiers out ______
...
# Lecture de la géométrie du domaine ___________________________________
...
# Ce qu'on va faire ____________________________________________________
do_the_chi = True
# ______________________________________________________________________
...
integ_S = { "x":0, "y":0, "z":0, "norm":0, "null":False }
integ_T = { "x":0, "y":0, "z":0, "norm":0, "null":False }

Taylor_S = { "x":0, "y":0, "z":0, "norm":0, "null":False }
Taylor_T = { "x":0, "y":0, "z":0, "norm":0, "null":False }
    ...
    for j,velocity in enumerate(direction_forces): 
        if velocity[-1].lower()==direction[0].lower(): 
            ...
            if len(np.where(U_corr_S2_norm<=0)[0]) != 0:
                integ_S[direction[0].lower()] = simps(U_corr_S2_norm[0:np.where(U_corr_S2_norm<=0)[0][0]],espace_corr[2:2+np.where(U_corr_S2_norm<=0)[0][0]])
            if len(np.where(U_corr_T2_norm<=0)[0]) != 0:
                integ_T[direction[0].lower()] = simps(U_corr_T2_norm[0:np.where(U_corr_T2_norm<=0)[0][0]],temps_corr[0:np.where(U_corr_T2_norm<=0)[0][0]])
            
            integ_S["norm"] += integ_S[direction[0].lower()]**2
            integ_T["norm"] += integ_T[direction[0].lower()]**2
            
            ## f(x+dx) -2f(x) + f(x-dx) = dx**2 f''(x)
            # ==> pour f paire et pour x=0 : 2 (f(dx)-f(0)) = dx**2 f''(0)
            # ==> L_taylor = (-0.5 f''(0)) ** (-1/2)
            Taylor_S[direction[0].lower()] = (-(2./(espace_corr[3]-espace_corr[2])**2) * (U_corr_S2_norm[1]-U_corr_S2_norm[0]))**(-1./2)
            Taylor_T[direction[0].lower()] = (-(2./(temps_corr[1]-temps_corr[0])  **2) * (U_corr_T2_norm[1]-U_corr_T2_norm[0]))**(-1./2)
            
            Taylor_S["norm"] += Taylor_S[direction[0].lower()]**2
            Taylor_T["norm"] += Taylor_T[direction[0].lower()]**2
...
integ_S["norm"]  = integ_S["norm"] **(0.5)
integ_T["norm"]  = integ_T["norm"] **(0.5)
Taylor_S["norm"] = Taylor_S["norm"]**(0.5)
Taylor_T["norm"] = Taylor_T["norm"]**(0.5)

integ_S["null"]  = not(integ_S["norm"] )
integ_T["null"]  = not(integ_T["norm"] )
Taylor_S["null"] = not(Taylor_S["norm"])
Taylor_T["null"] = not(Taylor_T["norm"])

if integ_S["null"]  :  integ_S["norm"]  = 1
if integ_T["null"]  :  integ_T["norm"]  = 1
if Taylor_S["null"] :  Taylor_S["norm"] = 1
if Taylor_T["null"] :  Taylor_T["norm"] = 1
...
for i,direction in enumerate(direction_sondes):
    for j,velocity in enumerate(direction_forces): 
        ...
        ax_Val.plot(temps[1:]/integ_T["norm"],
        ...
        ax_Val_smooth_mean.plot(temps[1:]/integ_T["norm"],
                                U[0][i][j].fluctuations_T2[2,:,int(U[0][i][j].npoints/2)],
                                linewidth=0.8,
                                color=color[velocity[-1].lower()],
                                linestyle=linestyle[direction[0].lower()])#,label=str(label_sonde))

        ax_CorT.plot(temps_corr[0:len(U_corr_T_norm)]/integ_T["norm"],
                     U_corr_T_norm,
                     linewidth=0.8,
                     color=color[velocity[-1].lower()],
                     linestyle=linestyle[direction[0].lower()])#,label=str(label_sonde))
        ax_CorT2.plot(temps_corr[0:len(U_corr_T2_norm)]/integ_T["norm"],
                      U_corr_T2_norm,
                      linewidth=0.8,
                      color=color[velocity[-1].lower()],
                      linestyle=linestyle[direction[0].lower()])#,label=str(label_sonde))
        ...
        if direction[0].lower()==velocity[-1].lower():
            ...
            if len(np.where(U_corr_T2_norm<=0)[0]) != 0:
                ax_CorT2_diag.plot(temps_corr[0:len(U_corr_T2_norm)]/integ_T["norm"],
                                    U_corr_T2_norm,
                                    linewidth=1.2,
                                    color=color[velocity[-1].lower()],
                                    linestyle=linestyle[direction[0].lower()])#,label=str(label_sonde))
                ax_CorT2_diag.scatter(temps_corr[np.where(U_corr_T2_norm<=0)[0][0]]/integ_T["norm"],
...
ax_Val.set_xlabel(r'$\frac{t}{\vert T_{int} \vert }$',fontsize=24)
ax_Val_smooth_mean.set_xlabel(r'$\frac{t}{\vert T_{int} \vert }$',fontsize=24)
ax_CorT.set_xlabel(r'$\frac{t}{\vert T_{int} \vert }$, $T_{int} = %f$'%(integ_T["norm"]),fontsize=24)
ax_CorT2.set_xlabel(r'$\frac{t}{\vert T_{int} \vert }$',fontsize=24)
ax_CorT2_diag.set_xlabel(r'$\frac{t}{\vert T_{int} \vert }$',fontsize=24)
...
for i,direction in enumerate(direction_sondes):
    for j,velocity in enumerate(direction_forces): 
        ...
        ax_Val.plot(espace/integ_S["norm"],
        ...
        ax_CorS.plot(espace_corr/integ_S["norm"],
                     U_corr_S_norm,
                     linewidth=0.8,
                     color=color[velocity[-1].lower()],
                     linestyle=linestyle[direction[0].lower()])#,label=str(label_sonde))
        ax_CorS2.plot(espace_corr/integ_S["norm"],
                      U_corr_S2_norm,
                      linewidth=0.8,
                      color=color[velocity[-1].lower()],
                      linestyle=linestyle[direction[0].lower()])#,label=str(label_sonde))
        ...
        if velocity[-1].lower()==direction[0].lower():
            # Le terme diagonal croise bien l'axe des abscisses
            if len(np.where(U_corr_S2_norm<=0)[0]) != 0:
                ax_CorS2_diag.plot(espace_corr/integ_S["norm"],
                                   U_corr_S2_norm,
                                   linewidth=1.2,
                                   color=color[velocity[-1].lower()],
                                   linestyle=linestyle[direction[0].lower()])#,label=str(label_sonde))
                ax_CorS2_diag.scatter(espace_corr[2+np.where(U_corr_S2_norm<=0)[0][0]]/integ_S["norm"],
...
ax_Val.set_xlabel(r"$\frac{x}{\vert L_{int} \vert}$, $\frac{y}{\vert L_{int} \vert}$ ou $\frac{z}{\vert L_{int} \vert}$, $\vert L_{int} \vert = %f$ (m)"%(integ_S["norm"]),fontsize=24)
ax_CorS.set_xlabel(r"$\frac{x}{\vert L_{int} \vert}$, $\frac{y}{\vert L_{int} \vert}$ ou $\frac{z}{\vert L_{int} \vert}$, $\vert L_{int} \vert = %f$ (m)"%(integ_S["norm"]),fontsize=24)
ax_CorS2.set_xlabel(r"$\frac{x}{\vert L_{int} \vert}$, $\frac{y}{\vert L_{int} \vert}$ ou $\frac{z}{\vert L_{int} \vert}$, $\vert L_{int} \vert = %f$ (m)"%(integ_S["norm"]),fontsize=24)
ax_CorS2_diag.set_xlabel(r"$\frac{x}{\vert L_{int} \vert}$, $\frac{y}{\vert L_{int} \vert}$ ou $\frac{z}{\vert L_{int} \vert}$, $\vert L_{int} \vert = %f$ (m)"%(integ_S["norm"]),fontsize=24)
ax_Val.set_ylabel(       r"$ %s_{ph}$"%(quelle_grandeur),fontsize=24)        
