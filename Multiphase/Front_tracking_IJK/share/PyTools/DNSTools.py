...
def getBarys(head=None):
   ...
   print(lmin)
   coords = array([x[:lmin,:], y[:lmin,:], z[:lmin,:]])
   _, nb = shape(x)
   #print(coords)
...
def getStats(caract = "moy"):
   if caract == "moy":
      ...
      ltimes = array([float(f.strip("monodiphasique_statistiques_.txt")) for f in fic_stats])
   else:
      ...
      ltimes = array([float(f.strip("monodiphasique_moyenne_spatiale_.txt")) for f in fic_stats])
      pass
   if (fic_stats == []): 
...
def evaluateRetau(U, z, jdd=None):
   ...
   #
   tauw = (tauwp+tauwm)/2.
   utau = array([sqrt(abs(x)/rhol) for x in tauw])
...
def test_cell_to_cell_gradient(z,Lz):
    ...
    u = array([sin(2.*pi*zz/Lz) for zz in z])
    nz = len(u) 
    dz = Lz/nz
    dudz = cell_to_cell_gradient(u,dz,0) # bc_type = champ nul au bord
    dudz_ana = array([2.*pi/Lz*cos(2.*pi*zz/Lz) for zz in z])
    ...
    w = array([cos(2.*pi*zz/Lz)+10 for zz in z])
    nz = len(w) 
    dz = Lz/nz
    dwdz = cell_to_cell_gradient(w,dz,2) # bc_type = derivee normale du champ nulle au bord
    dwdz_ana = array([-2.*pi/Lz*sin(2.*pi*zz/Lz) for zz in z])
    ...
    v = array([sin(2.*pi*zz/Lz)+10 for zz in z])
    nz = len(v) 
    dz = Lz/nz
    dvdz = cell_to_cell_gradient(v,dz,1) # bc_type = quelconque
    dvdz_ana = array([2.*pi/Lz*cos(2.*pi*zz/Lz) for zz in z])
...
def test_cell_to_cell_second_gradient(z,Lz):
    # Test d'une fonction nulle aux bords :
    u = array([sin(2.*pi*zz/Lz)+zz*(Lz-zz)*1000. for zz in z])
    nz = len(u) 
    dz = Lz/nz
    dduddz = cell_to_cell_second_gradient(u,dz,0) # bc_type = champ nul au bord
    dduddz_ana = array([-(2.*pi/Lz)**2*sin(2.*pi*zz/Lz)-2000. for zz in z])
    ...
    #
    # Test d'une fonction de derivee normale nulle aux bords :
    w = array([cos(2.*pi*zz/Lz)+1000 for zz in z])
    nz = len(w) 
    dz = Lz/nz
    ddwddz = cell_to_cell_second_gradient(w,dz,2) # bc_type = derivee normale du champ nulle au bord
    ddwddz_ana = array([-(2.*pi/Lz)**2*cos(2.*pi*zz/Lz) for zz in z])
    ...
    #
    # Test d'une fonction sans propriete aux bords : 
    v = array([sin(2.*pi*zz/Lz)+10+1000.*zz for zz in z])
    nz = len(v) 
    dz = Lz/nz
    ddvddz = cell_to_cell_second_gradient(v,dz,1) # bc_type = quelconque
    ddvddz_ana = array([-(2.*pi/Lz)**2*sin(2.*pi*zz/Lz) for zz in z])
...
# Option compo=-1 for getting a flag (just to know if it's there)
def getParam(fic, name, vec=False, compo=0, string=False):
   ...
   if len(goodline)==1:
      if vec:
         ...
         if (goodline[0].split()[0] == name):
            val=True
         else:
            print("The flag is there but not in first position! Return False.")
            val=False
         return val
      else:
