...
## ATTEMPT ANNAMISA I*b considered just once ###
dudu_l = dudu0 + I*b -cc -cc_t # The following plot validates the switch between variables you proposed
tracer([dudu_l_old ,dudu_l ], 'dissipation_x', [0])
tracer([dudu_l_old ,dudu_l ], 'dissipation_y' , [1])
tracer([dudu_l_old ,dudu_l ], 'dissipation_z', [2])
...
Rijk_l=fluctuijk(uuull, ull, uull, I).tensorProduct(I) # It's not really a tensor product as it is Tensor*Scalar ## I did on the classical variable putting alpha inside to avoid the multiplication after
...
#essay1=gradRijk.contract('ij','kijk') #N.B. The three contractions are not equal. Which one is correct in the equation??? 
#essay2=gradRijk.contract('ij','ijkk') 
# They are equal to me, up to 1e-15 tolerance. 
nonlin4=divRijk_l 
# Relative differences are of 1e-3 so not visible on a plot.
...
### Attempt sign!!!! The field where not loaded as I thought ####
nonlin2_old=nonlin2_old.contract('ij','ikjk')
...
#New attempt ##
test1=pression_l_ext*dull
test2 = pression_ll_ext*I*dull
test3 = pression_ll_ext*Idu
p2ijk_new =pdu_l_ext -test3 -test1 + test2
tracer([p2ijk_new ], 'red')
tracer([p2ijk,p2ijk_new ], 'red_y', [4])
tracer([p2ijk,p2ijk_new ], 'red_z', [8])
################## Molecular diffusion #############
gradRij=Rij_l.grad() 
ddRij0=gradRij.grad()
...
viscinter1_old=(visc1-visc2+visc3)
...
term3=term3.contract('ij', 'ijkk') ###Annamisia says: Yes because you transposed the loaded field ##
# I think it's a '+' before term2...
### Annamisia says:no I think it should be a - because of the decomposition for the fluctuations ###
...
c=c.contract('ij', 'ikjk')  ### ok indices 
c = c + c.transpo()
visc2=b+c
#e=ull.tensorProduct(aiN)
#f=Idu_l.tensorProduct(e)    
...
tracer([viscinter1_old,viscinter1], 'check_interface_x', [0])
tracer([viscinter1_old,viscinter1], 'check_interface_y', [4])
tracer([viscinter1_old,viscinter1], 'check_interface_z', [8])
...
##### It should be - because I changed the sign of the residue so that it this like your rigth hand side 
res_tot= residu - m
#res_tot= residu + m
