...
"""
PDF des vitesses dans la phase liquide a partir des sondes 
"""

##############################################################################
# mode d'emploi : python correlation_vII.py chemin_son  pattern  N_smooth
#                            0                1            2       3    
...
# /!\ 3 : Epaisseur du filtre pour moyenner le signal temporel 
...
# /!\ 4 : direction_sondes -> DOIT etre une liste, meme si on ne traite que les sondes X
# /!\ 4 : direction_forces -> DOIT etre une liste, meme si on ne traite que les vitesses X
# /!\ 6 : [temps0,temps1] -> releve les donnees des outs entre les 2 temps
# /!\ 6 : [temps0,temps0] -> releve la donnee des outs a l'instant le plus proche de temps0
...
pattern = sys.argv[2]
N_smooth = int(sys.argv[3])
direction_sondes = sys.argv[4].rstrip("]").lstrip("[").split(",") 
...
direction_forces = sys.argv[5].rstrip("]").lstrip("[").split(",")
# Ajustement des vitesses a traiter
for i, dv in enumerate(direction_forces):
    direction_forces[i] = 'F'+dv 
...
for i,direction in enumerate(direction_sondes):
    for j,velocity in enumerate(direction_forces):
        ...
        for u in U : 
            ...
            u[i][j].nom_sonde = velocity 
            u[i][j].nom_fichier = 'DNS_swarm_fin2'
...
fig_CorT.suptitle(r"Autocorrelation $\frac{\langle f_i f_i \rangle}{\langle f_i ^2 \rangle}$ sondes - Temporel",fontsize=24)
fig_CorT2.suptitle(r"Autocorrelation $\frac{\langle f_i f_i \rangle}{\langle f_i ^2 \rangle}$ sondes - Temporel - normal",fontsize=24)
fig_CorT2_diag.suptitle(r"Autocorrelation $\frac{\langle f_i f_i \rangle}{\langle f_i ^2 \rangle}$ sondes - Temporel - normal",fontsize=24)
    ...
    for j,velocity in enumerate(direction_forces): 
        ...
        if direction[0].lower()==velocity[-1].lower():
            ...
            if len(np.where(U_corr_T2_norm<=0)[0]) != 0:
                # print("BINGOW")
                # print("np.where(U_corr_T2_norm<=0) : ",np.where(U_corr_T2_norm<=0)[0])
...
ax_Val.set_ylabel(            r"$ F_{ph}$",fontsize=24)        
ax_Val_smooth_mean.set_ylabel(r"$ F_{ph}$",fontsize=24)        
ax_CorT.set_ylabel(           r"$\langle F_{ph}; F_{ph} \rangle$",fontsize=24)        
ax_CorT2.set_ylabel(          r"$\langle F_{ph}; F_{ph} \rangle$",fontsize=24)        
ax_CorT2_diag.set_ylabel(     r"$\langle F_{ph}; F_{ph} \rangle$",fontsize=24)        
...
fig_Val.savefig("Force_T.png")     
fig_Val_smooth_mean.savefig("Force_T_mean.png")     
fig_CorT.savefig("Correlation_T.png")     
fig_CorT2.savefig("Correlation_T2.png")     
fig_CorT2_diag.savefig("Correlation_T2_diagonaux.png")     
...
print("####")      
print("# Statistiques \"conventionnelles\"")
print('# Marquées (c) pour \'conventionnelles\'')
print("# Liste des sondes : "+str(liste_label_sonde))
print("# (c) Liste des moyennes : "+str(liste_moyenne_sondes_norm))
print("# (c) Liste des variances : "+str(liste_variance_sondes_norm))
print("# (c) Liste des skewness (moment d'o 3) : "+str(liste_skewness_sondes_norm))
print("# (c) Liste des kurtosis (moment d'o 4) : "+str(liste_kurtosis_sondes_norm))
print("####")
print("# Statistiques \"conventionnelles\" des sondes  - arrondis")
print('# Marquées (ca) pour \'conventionnelles, arrondies\'')
...
fig_CorS.suptitle(r"Autocorrelation sondes $\frac{\langle f_i f_i \rangle}{\langle f_i ^2 \rangle}$ - Spatial",fontsize=24)
fig_CorS2.suptitle(r"Autocorrelation sondes $\frac{\langle f_i f_i \rangle}{\langle f_i ^2 \rangle}$ - Spatial - normal",fontsize=24)
fig_CorS2_diag.suptitle(r"Autocorrelation sondes $\frac{\langle f_i f_i \rangle}{\langle f_i ^2 \rangle}$ - Spatial - normal",fontsize=24)
...
ax_Val.set_ylabel(       r"$ F_{ph}$",fontsize=24)        
ax_CorS.set_ylabel(      r"$\langle F_{ph}; F_{ph} \rangle$",fontsize=24)        
ax_CorS2.set_ylabel(     r"$\langle F_{ph}; F_{ph} \rangle$",fontsize=24)        
ax_CorS2_diag.set_ylabel(r"$\langle F_{ph}; F_{ph} \rangle$",fontsize=24)        
ax_Val.grid(True,which='both')        
ax_CorS.grid(True,which='both')        
ax_CorS2.grid(True,which='both')        
ax_CorS2_diag.grid(True,which='both')        
fig_Val.savefig("Force_S.png")     
fig_CorS.savefig("Correlation_S.png")     
fig_CorS2.savefig("Correlation_S2.png")     
fig_CorS2_diag.savefig("Correlation_S2_diagonaux.png")     
...
print("# Liste des kurtosis (moment d'o 4) : "+str(liste_kurtosis_sondes))
print("####")
print("# Statistiques des sondes \" conventionnel \"")
print("# (c) Liste des sondes : "+str(liste_label_sonde))
print("# (c) Liste des moyennes : "+str(liste_moyenne_sondes_norm))
print("# (c) Liste des variances : "+str(liste_variance_sondes_norm))
print("# (c) Liste des skewness (moment d'o 3) : "+str(liste_skewness_sondes_norm))
print("# (c) Liste des kurtosis (moment d'o 4) : "+str(liste_kurtosis_sondes_norm))
...
for i,direction in enumerate(direction_sondes):
    for j,velocity in enumerate(direction_forces): 
        if velocity[-1].lower()==direction[0].lower():  
            ...
            if len(np.where(U_corr_T2_norm<=0)[0]) != 0:
                integ_S= np.sum(U_corr_S2_norm[0:np.where(U_corr_S2_norm<=0)[0][0]]*espace_corr[2:2+np.where(U_corr_S2_norm<=0)[0][0]])
                integ_T= np.sum(U_corr_T2_norm[0:np.where(U_corr_T2_norm<=0)[0][0]]*np.array(temps_corr[0:np.where(U_corr_T2_norm<=0)[0][0]]))
            ...
            if len(np.where(U_corr_T2_norm<=0)[0]) != 0:
                ...
                print("L'autocorrelation normee ne s'annule pas...")
            #
            ...
            print("L_t : "+str(Taylor_S))
            print("T_t : "+str(Taylor_T))
