...
########################################################################
def extract_velocities_from_txt_file(chemin_fichier=liste_chemin[0],phases="vap",composantes="x"):
    ## Preparation nom generique des fichiers utilises
    fstat = glob.glob(chemin_fichier+"/diphasique_moyenne_spatiale_*.txt")
    if fstat == []:
        fstat = glob.glob(chemin_fichier+"/monophasique_moyenne_spatiale_*.txt")
    ...
    for f in fstat:
        ...
        if ((t[-1]>=t_deb) and (t[-1]<=t_fin)): 
            if (("liq" in phases) or ("all" in phases)):
                ...
                if ("x" in composantes) : u_l = np.loadtxt(f,usecols=(n_col_uI[0])).T
                if ("y" in composantes) : v_l = np.loadtxt(f,usecols=(n_col_uI[1])).T
                if ("z" in composantes) : w_l = np.loadtxt(f,usecols=(n_col_uI[2])).T
                # Moyenne spatiale
                m_I = np.mean(I,axis=-1)
                if ("x" in composantes) : m_u_l = np.mean(u_l,axis=-1)
                if ("y" in composantes) : m_v_l = np.mean(v_l,axis=-1)
                if ("z" in composantes) : m_w_l = np.mean(w_l,axis=-1)
                # Moyenne de phase
                if ("x" in composantes) : m_u_ll = m_u_l/m_I  
                if ("y" in composantes) : m_v_ll = m_v_l/m_I  
                if ("z" in composantes) : m_w_ll = m_w_l/m_I  
                # Ajout à la liste
                if ("x" in composantes) : UL.append(m_u_ll);
                if ("y" in composantes) : VL.append(m_v_ll);
                if ("z" in composantes) : WL.append(m_w_ll)
    
            if (("vap" in phases) or ("all" in phases)):
                # Lecture
                v = 1 - np.loadtxt(f,usecols=(n_col_I)).T
                if ("x" in composantes) : u_v = np.loadtxt(f,usecols=(n_col_uIv[0])).T
                if ("y" in composantes) : v_v = np.loadtxt(f,usecols=(n_col_uIv[1])).T
                if ("z" in composantes) : w_v = np.loadtxt(f,usecols=(n_col_uIv[2])).T
                # Moyenne spatiale
                m_v = np.mean(v,axis=-1)
                if ("x" in composantes) : m_u_v = np.mean(u_v,axis=-1)
                if ("y" in composantes) : m_v_v = np.mean(v_v,axis=-1)
                if ("z" in composantes) : m_w_v = np.mean(w_v,axis=-1)
                # Moyenne de phase
                if ("x" in composantes) : m_u_vv = m_u_v/m_v
                if ("y" in composantes) : m_v_vv = m_v_v/m_v
                if ("z" in composantes) : m_w_vv = m_w_v/m_v
                # Ajout à la liste
                if ("x" in composantes) : UV.append(m_u_vv);
                if ("y" in composantes) : VV.append(m_v_vv);
                if ("z" in composantes) : WV.append(m_w_vv)
        
        else:
            t.pop()
            
    return(t, UL, VL, WL, UV, VV, WV)
########################################################################
...
if "qdm" in figname :
    ax.set_title('QdM, moyenne de phase '+r' : $\overline{\rho U_i}^k = \frac{\overline{\chi_k \rho_k U_i}}{\overline{\chi_k}}$',fontsize=18)
    ax_smt.set_title('QdM, moyenne de phase lissees en temps'+r' : $\overline{\rho U_i}^k = \frac{\overline{\chi_k \rho_k U_i}}{\overline{\chi_k}}$',fontsize=18)
    ax_corr.set_title('QdM, moyenne de phase '+r' : $\overline{\rho U_i}^k = \frac{\overline{\chi_k \rho_k U_i}}{\overline{\chi_k}}$, corrigees',fontsize=18)
    ax_cp.set_title('QdM, moyenne de phase '+r' : $\overline{\rho U_i}^k = \frac{\overline{\chi_k \rho_k U_i}}{\overline{\chi_k}}$, cor+pentes',fontsize=18)
    ax_pente.set_title('QdM, moyenne de phase '+r' : $\overline{\rho U_i}^k = \frac{\overline{\chi_k \rho_k U_i}}{\overline{\chi_k}}$, pentes',fontsize=18)    
else:
...
for ind, jdd in enumerate(liste_jdd) :
    ...
    # = AFFICHAGE DIVERSE POUR L'UTILISATEUR
    print("######################################")
    ...
    mu_l = dtool.getParam(jdd, "mu_liquide")
    rho_l = dtool.getParam(jdd, "rho_liquide")
    nu_l = mu_l/rho_l
    if "vap" in phases :
        mu_v = dtool.getParam(jdd, "mu_vapeur")
        rho_v = dtool.getParam(jdd, "rho_vapeur")
        nu_v = mu_v/rho_v
    print("# Repertoire des fichiers.txt : "+liste_chemin[ind])#fstat)
    print("# Fichier.data : "+jdd)
    print("# viscosite cinematique : "+str(nu_l))
    print("######################################")
    ####################################################################
    
    # = GENERATION DES DONNES : temps, vitesses ou QdM liquide, vitesses ou QdM vapeur
    t, UL, VL, WL, UV, VV, WV = extract_velocities_from_txt_file(liste_chemin[ind],phases,composantes)
    if "qdm" in figname :
        """ 
            Si on veut des qdm, les listes U ne sont plus des vitesses.
            Les utilisateur d'IJK ne seront pas deroutes par cette maneouvre
        """
        UL, VL, WL = rho_l*np.array(UL), rho_l*np.array(VL), rho_l*np.array(WL)
        if "vap" in phases:
            UV, VV, WV = rho_v*np.array(UV), rho_v*np.array(VV), rho_v*np.array(WV)
    ####################################################################
    ...
    if (("x" in composantes) and ("liq" in phases)):
        ...
        ## SMOOTH
        d_ul_smt = np.convolve(UL,filtre_pour_smooth,mode='same')[-1]-np.convolve(UL,filtre_pour_smooth,mode='same')[0]
        d_uldt_smt = d_ul_smt/dt; puiss_smt = np.floor(mt.log10(abs(d_uldt_smt)))
        print("# smooth #")
        print("# Derive u_x liquide : d_ul = "+str(d_ul_smt))
        print("                       d_ul/dt = "+str(d_uldt_smt)) 
        ax_smt.plot(t, np.convolve(UL,filtre_pour_smooth,mode='same'),'r--',label=r"$\widetilde{\overline{U_x}^l}$; $\partial_t \widetilde{\overline{U_x}^l} = $"
                                  +str(np.round(d_uldt_smt*10**(-puiss_smt),2))+r"$\times 10^{%s}$"%(puiss_smt)
                                +" : "+str(liste_signes[ind]),
                           linewidth=linewidth[ind],
                           alpha=alpha[ind])                           
    ...
    if (("y" in composantes) and ("liq" in phases)):
        ...
        ## SMOOTH
        d_vl_smt = np.convolve(VL,filtre_pour_smooth,mode='same')[-1]-np.convolve(VL,filtre_pour_smooth,mode='same')[0]
        d_vldt_smt = d_vl_smt/dt; puiss_smt = np.floor(mt.log10(abs(d_vldt_smt)))
        print("# smooth #")
        print("# Derive v_x liquide : d_vl = "+str(d_vl_smt))
        print("                       d_vl/dt = "+str(d_vldt_smt)) 
        ax_smt.plot(t, np.convolve(VL,filtre_pour_smooth,mode='same'),'r--',label=r"$\widetilde{\overline{V_x}^l}$; $\partial_t \widetilde{\overline{V_x}^l} = $"
                                  +str(np.round(d_vldt_smt*10**(-puiss_smt),2))+r"$\times 10^{%s}$"%(puiss_smt)
                                +" : "+str(liste_signes[ind]),
                           linewidth=linewidth[ind],
                           alpha=alpha[ind]) 
    ...
    if (("z" in composantes) and ("liq" in phases)):
        ...
        ## SMOOTH
        d_wl_smt = np.convolve(WL,filtre_pour_smooth,mode='same')[-1]-np.convolve(WL,filtre_pour_smooth,mode='same')[0]
        d_wldt_smt = d_wl_smt/dt; puiss_smt = np.floor(mt.log10(abs(d_wldt_smt)))
        print("# smooth #")
        print("# Derive w_x liquide : d_ul = "+str(d_wl_smt))
        print("                       d_ul/dt = "+str(d_wldt_smt)) 
        ax_smt.plot(t, np.convolve(WL,filtre_pour_smooth,mode='same'),'r--',label=r"$\widetilde{\overline{W_x}^l}$; $\partial_t \widetilde{\overline{W_x}^l} = $"
                                  +str(np.round(d_wldt_smt*10**(-puiss_smt),2))+r"$\times 10^{%s}$"%(puiss_smt)
                                +" : "+str(liste_signes[ind]),
                           linewidth=linewidth[ind],
                           alpha=alpha[ind])
    ...
    if (("y" in composantes) and ("vap" in phases)):
        ...
        # Ajout des pentes (en l'etat du 07.07.21, si on ne donne pas temoin dans la liste des signes, la figure de correction n'aura que les pentes initiales et finales)
      #  pente_init, pente_fina = 0, 0
      #  for i in range(int(len(t)/5)):
      #      pente_init += (VL[i]-VL[i+1]) / (t[i]-t[i+1])
      #      pente_fina += (VL[-i-1]-VL[-i]) / (t[-i-1]-t[-i])               
    ...
    # = GESTION DES NOMS DES AXES, LINES, VALEURS SUR LES AXES ...
    ...
    # else:
    if "qdm" in figname:
        ax.set_ylabel(r'$\overline{\rho U_i}^k (m.s^{-1})$',fontsize=18)
        ax_smt.set_ylabel(r'$\overline{\rho U_i}^k (m.s^{-1})$',fontsize=18)
        ax_corr.set_ylabel(r'$\overline{\rho U_i}^k (m.s^{-1})$ corrigees',fontsize=18)
        ax_cp.set_ylabel(r'$\overline{\rho U_i}^k (m.s^{-1})$ corrigees',fontsize=18)
        ax_pente.set_ylabel(r'$\overline{\rho U_i}^k (m.s^{-1})$ corrigees',fontsize=18)
    else:
...
# = GESTION DE L'ESTHETIQUES DES FIGURES
