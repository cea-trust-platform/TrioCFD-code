...
def getFlag(fic, name, vec=False, compo=0, string=False):
    f=open(fic)
    lines = f.readlines()
    f.close()
    for i, line in enumerate(lines):
        # S'il y a du commentaire dans la ligne, on le supprime
        if line.find("#")>=0:
            lp = line.find("#")
            rp = line.rfind("#")
            if rp>lp:
                # Le marqueur existe dans le data. Il est commente.
                if name in line[lp-1:rp+1]:
                    val = name
                s = line[:lp-1]+line[rp+1:]
                # print(lines[i], "replaced by ", s)
                lines[i]= s
    ...
    # goodline=[line for line in goodline if "#" not in line]
    # print(goodline,val)
    # = On a des lignes ou le flag n'est pas commente
    if len(goodline)==1:
        if vec:
            val=goodline[0].split()[2+compo]
        else:
            val=goodline[0].split()[0]
        try:
            return val==name
        except:
            if (not string): raise Exception('Trying to return a string? %s'%val)
        ...
        # = On n'a pas de lignes ou le flag est commente, mais il existe en commente
        return 0
    else:
        # = Le flag n'est pas present du tout. La sortie est aussi 0 en ft
        return(0)
    return 0/0
