...
#echo "S1"=$S1
#echo "S2"=$S2
V1=`grep "cl_39 frontiere_ouverte_vitesse_imposee Champ_Front_Uniforme" $1.data| awk '{print $7}' | head -1`
...
tail -2  $1_SONDE_PRESSION.coupe > delta_p2

tail -1  delta_p1 > delta_p
tail -1  delta_p2 > delta_pp


P1_sur_rho_trio=`cat delta_p| awk '{print $2}' | head -1`
P1=`awk "BEGIN {print $P1_sur_rho_trio*$rho}" `

P2_sur_rho_trio=`cat delta_pp| awk '{print $2}' | head -1`
P2=`awk "BEGIN {print $P2_sur_rho_trio*$rho}" `
...
cat delta_p.dat >> ../deltap_courbe3d.dat
