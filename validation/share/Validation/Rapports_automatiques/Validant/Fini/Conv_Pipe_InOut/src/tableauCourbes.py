...
def properties():
    ...
    fic.close()
    return D,mu,lamb,Cp,rho,U,Qw,Z,L
...
def UbulkCalc(rho,D):
    ...
    tLigne = ligne.split()
    Ucalc=tLigne[1]
    Ucalc=float(Ucalc)/float(rho)/(float(Surf))
    ficUbulk.close()
    return Ucalc
        ...
        if ligne != "\n" and ligne != "":
            ...
            if commentaire!="----------------":
                ...
                if (Z-z) < abs(deltaz) and (Z-z)>0 :
                    ...
                    deltaz = Z-z
        i=i-1
    #on cherche la valeur x la plus proche de la paroi (X=-1) pour la cote Z
    commentaire=""
    i=len(tFic)
    X=-float(D)/2.
    valx=X
...
def Ttotal(L):
    # recherche des Tparoi en sortie : on fait la moyenne des valeurs trouvees
    # ouverture des fichiers
    nomFicTw = 'Conduite_pb_Nusselt.face'
    ...
    commentaire=""
    i=len(tFic)
    # on recherche les Tparoi et on les somme
    cumul=0
    nbTemps=0
    while commentaire!="----------------":
        ligne=tFic[i-1]
...
def DHtrio2(L):
    #on va calculer la perte de pression sur la 2eme moitie du canal
    #on estime que l'ecoulement est bien developpe dans cette partie

    #ouverture des fichiers
    nomFic = 'Conduite_SONDE_PB.coupe'
    ...
    while not fin:
        ...
        if not ligne:
            ...
            ymax = y
            pmax = tLigne[-1]
            if not trouve_min and (float(y) > float(L/2.)):
                ymin = y
                pmin = tLigne[-1]
                trouve_min = True

    DHtrio=abs(float(pmax)-float(pmin))/(float(ymax)-float(ymin))
...
if __name__ == '__main__':
    ...
    D,mu,lamb,Cp,rho,U,Qw,Z,L = properties()
