...
def yPlus(L):
    # ouverture des fichiers
    nomFicUstar = 'mixedBC_pb_Ustar.face'
...
def ecrire_profils_visco(sonde,u_tau,h):
    ...
    nomFic = sonde+'_Dimensionless.dat'
    ficWrite = open(nomFic, 'w')
...
if __name__ == '__main__':
    ...
    y_plus = yPlus(L)
    u_tau_trio=u_tau(L)
    Re_tau,Re_tau_theo=retau(rho,h,mu,u_tau_trio,Ub)
    ...
    #ecriture du profils de vitesse Vx=f(y) et U+=f(y+) en x=1.0
    fichierSonde='mixedBC_SONDE_VIT.coupe'
    ecrire_profils_vitesse(fichierSonde,u_tau_trio,mu,rho)

    #ecriture du profils de vitesse Vx=f(y) et U+=f(y+) en x=9.0
    fichierSonde='mixedBC_SONDE_VIT4.coupe'
    ecrire_profils_vitesse(fichierSonde,u_tau_trio,mu,rho)



    #ecriture du profils de viscosite turbulente adimensionnee
    fichierSonde2='mixedBC_SONDE_VISC_TURB1.coupe'
    ecrire_profils_visco(fichierSonde2,u_tau_trio,h)

    fichierSonde3='mixedBC_SONDE_VISC_TURB2.coupe'
    ecrire_profils_visco(fichierSonde3,u_tau_trio,h)
