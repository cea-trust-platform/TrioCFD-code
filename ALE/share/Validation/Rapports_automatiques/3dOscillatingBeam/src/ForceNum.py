...
Fy_adim = Fy/(rho*Ri**2.*u*w**2.) 

DataOut = np.column_stack((t_adim,Fy_adim))
np.savetxt('Numerical_force_adim.txt', DataOut)
...
CMNumy=2./nb_p/T*np.trapz(np.sin(w*t)*Fy,t)/(u*w**2.)
CVNumy =- 2./nb_p/T*np.trapz(np.cos(w*t)*Fy,t)/(u*w)


CmNumy = CMNumy/(Lmax*rho*Ri**2.*np.pi)
CvNumy = CVNumy/(Lmax*rho*Ri**2.*np.pi*w)
